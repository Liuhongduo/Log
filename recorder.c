...
int record(int section_id,int type,int key,int oper,int para_cnt,...)
{
    ...
    total_len+=sizeof(int);//the number of parameter
    char *para;
    ...
    record_element* contents=malloc(record_element_len+total_len+table[section_id].block_num);
    //Acquire system time
    struct tm *times;
    time_t t;
    t = time(0);
    ...
    for(int i=0;i<table[section_id].block_num;++i)
    {
        *x='\0';
        x++;
    }
     //if the record is too long to be contained by the whole section,then return -1
   int cnt=table[section_id].len/(table[section_id].block_num+record_node_len);
   if(contents->len>cnt*table[section_id].block_num) return -1;
   ...
   if(contents->len%table[section_id].block_num==0)
    block_covered=contents->len/table[section_id].block_num;
   else block_covered=contents->len/table[section_id].block_num+1;
   ...
   {
           //write memory
           memcpy(initial_addr+p->block_offset,q+table[section_id].block_num*i,table[section_id].block_num);
...
int record_section(int section_id,int block_size,void *addr,int len)
{
    ...
    table[section_id].block_num=block_size;
